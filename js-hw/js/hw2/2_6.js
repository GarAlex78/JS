...
let input;
const numberArrays = [];
const addNumberInArray = function(input) {
  input = Number(input);
  if (Number.isNaN(input)) {
    return alert("было введено не число, пожалуйста введите число");
  } else {
    numberArrays.push(input);
  }
};
const sumNumberInArray = function(numberArrays) {
  let sum = 0;
  for (let arrayItem of numberArrays) {
    sum += arrayItem;
  }
  return sum;
};
...
do {
  ...
  if (input !== null) {
    addNumberInArray(input);
  } else {
    console.log("Сумма всех чисел: ", sumNumberInArray(numberArrays));
    break;
  }
